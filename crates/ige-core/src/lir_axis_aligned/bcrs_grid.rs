//! Grid-based axis-aligned rectangle solvers used by the LIR Approximate Oriented pipeline.
//!
//! Port of `_solve_axis_rect_grid` (coarse grid, now using 1D Sobol quasi-random
//! grid lines for lower-discrepancy coverage) and `_solve_axis_rect_fine`
//! (vertex-coordinate grid) from `bcrs_fast_worker.py`.

use std::collections::HashSet;

use geo::{BoundingRect, Centroid, Contains, EuclideanDistance};
use geo_types::{Point, Polygon};
use ordered_float::OrderedFloat;

use crate::lir_axis_aligned::histogram::{lrih, lrih_vp};

// --- Vertex-coordinate grid (Daniels-style) --------------------------------

/// Build grid lines from polygon vertex coordinates, adding midpoints between
/// adjacent pairs for sparse coordinate sets.  Capped at `max_steps` on each axis.
fn vertex_grid(poly: &Polygon<f64>, max_steps: usize) -> (Vec<f64>, Vec<f64>) {
    let mut raw_x = HashSet::new();
    let mut raw_y = HashSet::new();
    for c in poly.exterior().0.iter() {
        raw_x.insert(OrderedFloat(c.x)); raw_y.insert(OrderedFloat(c.y));
    }
    ...
    let mut xv: Vec<f64> = raw_x.iter().map(|v| v.into_inner()).collect();
    let mut yv: Vec<f64> = raw_y.iter().map(|v| v.into_inner()).collect();
    ...
    // If vertex grid is sparse (< max_steps), fill with uniform grid points.
    let fill_to = |v: &mut Vec<f64>, lo: f64, hi: f64| {
        if v.len() >= max_steps { return; }
        let need = max_steps - v.len();
        let step = (hi - lo) / (max_steps as f64);
        for i in 1..=need {
            let t = i as f64 / (need + 1) as f64;
            let p = lo + t * (hi - lo);
            v.push(p);
        }
    ...
    if let Some(bb) = poly.bounding_rect() {
        fill_to(&mut xv, bb.min().x, bb.max().x);
        fill_to(&mut yv, bb.min().y, bb.max().y);
    }

    if xv.len() > max_steps { let s = xv.len() / max_steps; xv = xv.iter().step_by(s.max(1)).take(max_steps).copied().collect(); }
    if yv.len() > max_steps { let s = yv.len() / max_steps; yv = yv.iter().step_by(s.max(1)).take(max_steps).copied().collect(); }
    (xv, yv)
}

// --- Uniform-grid solver (coarse / Brent) ---------------------------------

/// Solve the largest axis-aligned rectangle using `grid_steps` grid lines
/// on each axis.  Grid lines are placed at vertex coordinates (Daniels-style).
...
pub fn solve_axis_rect_grid(
    ...
    // Use vertex coordinates as grid lines (Daniels-style) for boundary-aligned
    // rectangle boundaries.  Falls back to uniform grid if vertex count exceeds
    // AA_GRID_MAX_COORDS.
    let (xs, ys) = vertex_grid(poly, grid_steps);

    let n = xs.len();
    if n < 2 || ys.len() < 2 { return None; }

    // Build point-inside mask at each grid-point (tolerance-aware for boundary points)
    let ny = ys.len();
    let mut mask = vec![false; n * ny];
    for r in 0..ny {
        for c in 0..n {
            let pt = Point::new(xs[c], ys[r]);
            mask[r * n + c] = poly.contains(&pt) || poly.euclidean_distance(&pt) <= 1e-12;
        }
    }
...
pub fn solve_axis_rect_fine(
    ...
    build_axis_grid_mask(rot_poly, &xs_raw, &ys_raw, &mut mask, n_cols, n_rows);
...
/// of their crossing structure — only edges entering or leaving the active set
...
fn fallback_to_cpu_axis_grid_mask(poly: &Polygon<f64>, xs: &[f64], ys: &[f64], mask: &mut [bool], n_cols: usize, n_rows: usize) {
    for r in 0..n_rows {
...
fn build_axis_grid_mask(poly: &Polygon<f64>, xs_bounds: &[f64], ys_bounds: &[f64], mask: &mut [bool], n_cols: usize, n_rows: usize) {
    #[cfg(feature = "gpu")]
    {
    ...
    fallback_to_cpu_axis_grid_mask(poly, xs_bounds, ys_bounds, mask, n_cols, n_rows);
}
...
mod tests {
    ...
    #[test]
    fn axis_grid_finds_full_square() {
        let poly = unit_square();
        let result = solve_axis_rect_fine(&poly, None, 0.0);
        assert!(result.is_some());
        let (x0, _y0, x1, _y1, area) = result.unwrap();
