//! Brent angle-polish and LIR Approximate Oriented expansion.
//!
//! OpenEvolve target: ``--target lir_approximate_oriented/polish.rs --mode balanced``
//!
//! Stage 3: golden-section search for optimal angle within +/-POLISH_HALFWIDTH.
//! Stage 4+5: LIR Approximate Oriented vertex-coordinate solve + SDF-guided boundary expansion.
...
use crate::lir_axis_aligned::solve_axis_rect_fine;
use super::AngleCandidate;
use crate::lir_approximate_oriented::expand::expand_rect_to_boundary;
...
pub(crate) fn polish_angle(
    ...
    let mut neg_area = |a: f64| -> f64 {
        ...
        // Use LIR axis-aligned fine (vertex-coordinate-based, exact precision) instead of the
        // grid for polish evaluations.  The grid at GRID_COARSE=32 cannot
        // distinguish angles closer than ~1°; fine solve evaluates exact vertex
        // positions and SDF, giving sub-0.01° angular resolution.
        let area = match solve_axis_rect_fine(&rot, None, max_ratio) {
            Some((_, _, _, _, a)) => a,
            None => 0.0,
        };
...
/// Stage 4+5: LIR axis-aligned fine vertex-coordinate solve followed by SDF expansion.
pub(crate) fn expand_at_angle(
    rot_poly: &Polygon<f64>,
    seed_bounds: Option<(f64, f64, f64, f64)>,
    max_ratio: f64,
) -> Option<((f64, f64, f64, f64), f64)> {
    let fine_result = solve_axis_rect_fine(rot_poly, seed_bounds, max_ratio);
    let (bx0, by0, bx1, by1) = match fine_result {
